...
const priorityConfig: Record<Priority, { label: string; dot: string; badge: string }> = {
  alta: { label: "Alta", dot: "bg-red-400", badge: "bg-red-50 text-red-600 border-red-200" },
  media: { label: "Media", dot: "bg-amber-400", badge: "bg-amber-50 text-amber-600 border-amber-200" },
  baja: { label: "Baja", dot: "bg-emerald-400", badge: "bg-emerald-50 text-emerald-600 border-emerald-200" },
};

const statusConfig: Record<Status, { label: string; icon: typeof Check }> = {
  pendiente: { label: "Pendiente", icon: Clock },
  en_progreso: { label: "En Progreso", icon: AlertCircle },
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { key: "pendiente", label: "Pendientes", value: counts.pendiente, color: "text-amber-600", bg: "bg-amber-50" },
          { key: "en_progreso", label: "En Progreso", value: counts.en_progreso, color: "text-blue-600", bg: "bg-blue-50" },
          { key: "completada", label: "Completadas", value: counts.completada, color: "text-emerald-600", bg: "bg-emerald-50" },
        ].map((s) => (
          <motion.button
            key={s.key}
            whileHover={{ y: -1 }}
            onClick={() => setFilter(filter === s.key as Status ? "todas" : s.key as Status)}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <h3
                        className={`text-sm font-semibold text-slate-800 ${
                          task.status === "completada" ? "line-through text-slate-400" : ""
                        }`}
                      >
                        {task.title}
                      </h3>
                      <button className="p-1 text-slate-300 hover:text-slate-600 opacity-0 group-hover:opacity-100 transition-all shrink-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                      ...
                      {task.patient && (
                        <span className="text-xs text-violet-600 bg-violet-50 px-2 py-0.5 rounded-lg">
                          {task.patient}
                        </span>
                      )}
